...
/**
 * Servlet implementation class GymDiet
 */
public class GymDiet extends HttpServlet {
		...
		if (header != null) {
			...
			JavaRDD<String> words = data.toJavaRDD().flatMap(new FlatMapFunction<Row, String>() {
				...
				public Iterable<String> call(Row row) throws Exception {
					// TODO Auto-generated method stub
					String s = "";
					List<String> keywords = Arrays.asList("gym", "diet", "exercise", "workout", "protein", "yoga");
			...
			ctx.stop();
			request.setAttribute("gymdiet", sortedData);
			request.getRequestDispatcher("gymdiet.jsp").include(request, response);
		} else {
			try {
