...
/**
 * Servlet implementation class StreetFood
 */
public class StreetFood extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (header != null) {
			...
			SparkConf sparkConf = new SparkConf().setAppName("StreetFood").setMaster("local")
			...
			DataFrame data;
			data = sc.sql("select text from tweets where text LIKE	'%pizza%' or  text LIKE '%sandwich%' or  "
					+ "text LIKE '%pasta%' or  text LIKE '%salad%' or  text LIKE '%Chicken%' or "
					+ " text LIKE '%Mutton%' or  text LIKE '%fish%' or  text LIKE '%beef%' or "
					+ " text LIKE '%dosa%' or  text LIKE '%panipuri%' or  text LIKE '%vadapav%' or "
					+ " text LIKE '%aloo chat%' or  text LIKE '%momos%' or  text LIKE '%bhelpuri%'");
			JavaRDD<String> words = data.toJavaRDD().flatMap(new FlatMapFunction<Row, String>() {
				...
				public Iterable<String> call(Row row) throws Exception {
					// TODO Auto-generated method stub
					String s = "";
					List<String> keywords = Arrays.asList("pizza", "sandwich", "pasta", "salad", "salads", "chicken",
							"Mutton", "fish", "beef", "dosa", "panipuri", "vadapav", "aloo chat", "momos", "bhelpuri");
			...
			ctx.stop();
			request.setAttribute("streetfood", sortedData);
			request.getRequestDispatcher("streetfood.jsp").include(request, response);
		} else {
			try {
