...
/**
 * Servlet implementation class FluServlet
 */
public class FluServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (header != null) {
			...
			DataFrame data;
			data = sc
					.sql("select place.full_name,text from tweets where place.country='United States' and place.full_name IS NOT NULL and "
							+ "text LIKE '%flu%' or " + "text LIKE '%ill%' or " + "text LIKE '%sick%' or "
							+ "text LIKE '%disease%' or " + "text LIKE '%fever%' ");
			JavaRDD<String> words = data.toJavaRDD().flatMap(new FlatMapFunction<Row, String>() {
				...
				public Iterable<String> call(Row row) throws Exception {
					// TODO Auto-generated method stub
					String s = "";
					List<String> keywords = Arrays.asList("flu", "fever", "ill", "sick", "disease");

					String state_code;
					try {
						if (row.getString(0) != null) {
							String code = row.getString(0).split(",")[1].trim();
							if (code != null && code.length() == 2) {
								state_code = code;
								
								for (String str : keywords) {
									if (row.getString(1).toLowerCase().contains(str.toLowerCase()))
										s += " " + state_code;
								}
							}
						}
			...
			ctx.stop();
			request.setAttribute("flu", sortedData);
			request.getRequestDispatcher("flu.jsp").include(request, response);
		} else {
			try {
